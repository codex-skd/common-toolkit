...
public sealed interface DynamicHolderSet<R> permits DynamicHolderSet.Named, DynamicHolderSet.Direct {
    ...
    static <R> Codec<DynamicHolderSet<R>> codec(DynamicRegistry<R> registry) {
        Codec<DynamicTagKey<R>> tagKeyCodec = Codec.STRING.comapFlatMap(
            s -> {
                if (!s.startsWith("#")) {
                    return DataResult.error(() -> "Not a tag reference (must start with '#'): " + s);
                }
                ResourceLocation id = ResourceLocation.tryParse(s.substring(1));
                return id == null
                    ? DataResult.error(() -> "Invalid resource location: " + s.substring(1))
                    : DataResult.success(new DynamicTagKey<R>(registry.getId(), id));
            },
            key -> "#" + key.id());
