...
> Este archivo pertenece al proyecto **Common Toolkit**. Cambios aquí solo afectan a este proyecto.
...
|---|---|---|
| **snake_case** | `mod_id`, assets/, packages Java | `common_toolkit` |
| **PascalCase** | Clases Java principales | `CommonToolkit` |
| **camelCase** | Variables, métodos, config keys | `commonToolkitConfig` |
| **Title Case** | Display name (README, CHANGELOG, docs, CurseForge) | `Common Toolkit` |

Paquete base: `com.skd.commontoolkit` (equivalente Java del mod id, sin guiones bajos, siguiendo el mismo patrón que `utility_core` → `com.skd.utilitycore`).
...
`build.gradle` · `gradle.properties` (mod_id, mod_version, mod_group_id, mod_framework) · `settings.gradle` · `src/main/java/com/skd/commontoolkit/` · `src/main/resources/assets/common_toolkit/` · `src/main/resources/templates/META-INF/neoforge.mods.toml` (plantilla, gradle la expande) · `src/main/resources/META-INF/accesstransformer.cfg` (heredado de Placebo) · `libs/` (versionado) · `lib_ext/` y `temp/` (no versionados) · `docs/` (WORKFLOW + curseforge/) · `CHANGELOG.md` · `README.md` · `graphify-out/` (versionado).
...
- README en inglés siempre actualizado · sin residuos del mod original (paquetes, clases, toml, lang, assets) · atribución MIT explícita (README, NOTICE, project_description)
